...
const getHandlebarsFilesInFolder = path => filter(getFilenamesInFolder(path), isHandlebarsFile);
...
const createFolders = path => reduce(
...
const compileHandlebarsFile = (filename, inputFolder, outputFolder) => {
    ...
    const outputPath = join(outputFolder, filename.replace('.handlebars', '.html'));
    const template = readFileSync(filepath, 'utf-8');
    const context = JSON.parse(readFileSync(filepath.replace('.handlebars', '.json')));
...
const compileHandlebarsFilesInFolder = (inputFolder, outputFolder) => each(
    getHandlebarsFilesInFolder(inputFolder),
    filename => compileHandlebarsFile(filename, inputFolder, outputFolder)
);
